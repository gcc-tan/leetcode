...
分别为height[][], right[][], left[][]，来表示以这个点为底边的任意一点，向上连续’1‘的长度为轴向左向右，能够组成的面积最大的内部全部为’1‘的长方形。
...
注意代码实现过程中当本身值点(i, j)为'0'时，我们设置成left[i][j] = 0, right[i][j] = n，这是为了满足某些边界条件left，right左闭右开区间条件，例如matrix就是一个1*1的'1'
